...
def _wrapped_assrt(expr: str, to_check: bool, expected, real):
	try:
		assert(to_check)
...
def _test_suit(expr: str, expected_ok: bool, expected_res: float = 0.0):
	prec = 1e6
	res, ok = evaluate_expression(expr)
	_wrapped_assrt(expr, ok == expected_ok, expected_ok, ok)
	if (expected_ok and ok):
		test_result_l = expected_res - prec
		test_result_r = expected_res + prec
		_wrapped_assrt(expr, res >= test_result_l and res <= test_result_r, expected_res, res)
